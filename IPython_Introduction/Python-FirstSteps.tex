...
%  Use the help commands to locate the \pythoninline{scipy.stats} documentation on the Poisson distribution.
...
%Try calling the poisson distribution function from \pythoninline{scipy.stats} with no arguments.
...
%Write a function \pythoninline{data_avg(list, int)} that returns a new list $\bar{x}_n$, containing all possible averages of n values selected from the list.
