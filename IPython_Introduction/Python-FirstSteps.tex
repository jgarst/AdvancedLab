...
\centering{
  A quick start: \\
    \texttt{In~[1]:~help()} \\
    \texttt{In~[2]:~help(list)} \\
    \texttt{In~[3]:~?} \\
    \texttt{In~[4]:~import~numpy; \\
                   numpy?} \\
    \texttt{In~[5]:~numpy??} \\
    \texttt{In~[6]:~numpy.*cos*?} \\
    \texttt{In~[7]:~numpy.arc<tab>} \\
    \texttt{In~[8]:~Ctrl-m h}}} \\
...
you can check a type with \pythoninline{type(object)}, and enforce a
type with \pythoninline{assert} and \pythoninline{isinstance}.
\begin{adjustwidth}{2.5em}{0pt}
%\begin{lstlisting}[language=Python]
...
%\end{lstlisting}
\end{adjustwidth}
\pythoninline{assert} will stop the program with an error if the next statement is
